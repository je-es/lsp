...
var Path = __toESM(require("path"), 1);
var EventEmitter = __toESM(require("events"), 1);
        ...
        if (modulePath && !this.settings.isAnonymous) {
          ...
          for (const filePath of allKemetFiles) {
            ...
            if (!this.program.modules.has(otherModuleName)) {
              const module2 = this.parseModuleFromFile(filePath);
              if (module2) {
                this.program.modules.set(otherModuleName, module2);
  ...
  parseModuleFromFile(filePath) {
    try {
      ...
      if (parseResult.errors.length > 0) {
        ...
        const module22 = Module.create(moduleName, []);
        module22.metadata.path = filePath;
        module22.metadata.hasParseErrors = true;
        return module22;
      }
      this.moduleParseErrors.delete(filePath);
      const statements = parseResult.ast.length > 0 ? parseResult.ast[0].getCustomData() : [];
      const module2 = Module.create(moduleName, statements);
      ...
      for (const filePath of kemetFiles) {
        try {
          ...
          if (module2) {
            ...
            debugMgr.log("verbose", `[Project] Loaded module '${module2.name}' from ${filePath}`);
          }
        } catch (e) {
  ...
  handleDiagnostics(params) {
    return __async(this, null, function* () {
      try {
        ...
        if (!document) {
          return {
            kind: import_vscode_languageserver.DocumentDiagnosticReportKind.Full,
            items: []
          };
        }
  ...
  convertKemetDiagnostic(kemetDiag, document, settings) {
    ...
    try {
      ...
      if (kemetDiag.kind === "error") {
        ...
        severity = import_vscode_languageserver.DiagnosticSeverity.Information;
      }
      return {
        severity,
        range: {
...
var CompletionHandler = class {
  ...
  getKeywordCompletions(context) {
    ...
    keywords.forEach((keyword, index) => {
      const item = {
        label: keyword,
        kind: KEYWORDS.types.includes(keyword) ? import_vscode_languageserver2.CompletionItemKind.TypeParameter : KEYWORDS.controlFlow.includes(keyword) ? import_vscode_languageserver2.CompletionItemKind.Keyword : import_vscode_languageserver2.CompletionItemKind.Keyword,
        data: `keyword_${index}`,
        sortText: `0_${keyword}`
      };
      items.push(item);
  ...
  getBuiltinCompletions() {
    return KEYWORDS.builtins.map((builtin, index) => ({
      label: builtin,
      kind: builtin === "self" ? import_vscode_languageserver2.CompletionItemKind.Variable : import_vscode_languageserver2.CompletionItemKind.Function,
      data: `builtin_${index}`,
  ...
  determineProject(uri) {
    if (uri.startsWith("file://")) {
      try {
        const filePath = (0, import_url2.fileURLToPath)(uri);
  ...
  getCompletionItemKind(symbolKind) {
    switch (symbolKind) {
      case "Function":
        return import_vscode_languageserver2.CompletionItemKind.Function;
      case "Variable":
      ...
      case "EnumVariant":
        return import_vscode_languageserver2.CompletionItemKind.EnumMember;
      case "Use":
...
var HoverHandler = class {
  ...
  determineProject(uri) {
    if (uri.startsWith("file://")) {
      try {
        const filePath = (0, import_url3.fileURLToPath)(uri);
...
var KemetLSP = class {
  ...
  handleInitialize(params) {
    ...
    try {
      ...
      const result = {
        capabilities: {
          textDocumentSync: import_vscode_languageserver4.TextDocumentSyncKind.Incremental,
          completionProvider: {
  ...
  handleInitialized() {
    try {
      if (this.hasConfigurationCapability) {
        this.connection.client.register(import_vscode_languageserver4.DidChangeConfigurationNotification.type, void 0);
      }
      if (this.hasWorkspaceFolderCapability) {
